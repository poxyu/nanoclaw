...
async function convertToWav(inputPath: string): Promise<string> {
  ...
  await execFileAsync(
    ...
    [
      ...
      inputPath,
      '-ar',
      '16000', // 16 kHz sample rate (whisper requirement)
      '-ac',
      '1', // mono
      '-c:a',
      'pcm_s16le',
  ...
  try {
    ...
    const { stdout, stderr } = await execFileAsync(
      ...
      [
        ...
        WHISPER_MODEL,
        '--language',
        'auto', // auto-detect Russian/English
        '--threads',
        WHISPER_THREADS,
        '--beam-size',
        '5', // beam search for best quality
        '--best-of',
        '5', // evaluate 5 candidates per step
      ...
      { timeout: 120_000 },
    ); // 2 min timeout for long messages

    const transcript = stdout.trim();
    if (!transcript) {
      logger.warn(
        { audioPath, stderr: stderr.slice(0, 500) },
